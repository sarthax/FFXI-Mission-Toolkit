...
- [x] Client DAT/Binary Inspector GUI pages and feature-presence probes (`binary_probes.py`) emitting capability observations/requirements — see CLIENT_BINARY_RESEARCH.md; probe-set JSON files + read-only GUI runner (PoC); [ ] persist-from-GUI, probe-set editor, Client Overview/Build fingerprint page.

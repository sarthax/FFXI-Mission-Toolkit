...
9. Domain plugins now have a generalized extension interface and the reusable battlefield framework has active migration/reshape and battlefield representation planning. Safe role-scoped plugin reshape findings can refine generic MigrationActions without moving battlefield semantics into the core. SQL-backed policy/membership may be generated only when safe, while callback-body semantics remain evidence/manual-review driven. Assault-specific analyzers and other system plugins remain future work.

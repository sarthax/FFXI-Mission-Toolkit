...
9. Domain plugins now have a generalized extension interface. The reusable battlefield framework has active migration/reshape and battlefield representation planning, and mission representation planning can decompose modular source mission behavior into target lifecycle requirements across distributed legacy scripts. Safe role-scoped plugin reshape findings can refine generic MigrationActions without moving domain semantics into the core. Verified mission gaps may emit proposal-only mission gap artifacts that remain non-target-ready and force review. SQL-backed policy/membership may be generated only when safe, while callback/body mission semantics remain evidence/manual-review driven. Assault-specific analyzers and other system plugins remain future work.

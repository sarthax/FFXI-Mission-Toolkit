...
pub unsafe fn str_from_null_terminated_utf8_unchecked(s: &[u8]) -> &str {
    unsafe { std::str::from_utf8_unchecked(CStr::from_ptr(s.as_ptr() as *const _).to_bytes()) }
}
...
#[inline]
pub fn get_padding_8(len: usize) -> usize {
    (8 - (len & 7)) & 7
}

#[inline]
pub fn get_padding_16(len: usize) -> usize {
    (16 - (len & 15)) & 15
}

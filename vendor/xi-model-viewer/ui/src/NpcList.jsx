...
export function NpcList({
  onSelectEntry, selectedPath, onError, autoOpenFileId = null, autoOpenReady = true,
  onCatalogMiss = null,
}) {
  ...
  // Deep-link auto-select: an external "view this entity" link (Mission
  // Toolkit's Entity Lookup, or any other tool) opens this app on ?npc=<real
  // DAT file id> — see ui/js/launch.js. Fires once the real list is loaded
  // AND the caller says it's safe to (autoOpenReady — gamePath must be set
  // before onSelectEntry's eventual loadNpcEntry() tries to read DAT bytes),
  // guarded by a ref so a re-render (e.g. gamePath settling in) never re-fires
  // it and stomps on whatever the user has since clicked.
  const autoOpenedRef = useRef(false);
  useEffect(() => {
    if (autoOpenedRef.current) return;
    if (autoOpenFileId == null || !autoOpenReady || !allEntries.length) return;
    autoOpenedRef.current = true;
    const hit = allEntries.find((e) => e.entry.fileId === autoOpenFileId);
    if (!hit) {
      // Not in the curated list -- may still be a real, just uncatalogued DAT. Let the caller
      // (App.jsx) try a raw FTABLE/VTABLE resolution instead of reporting "not found" outright;
      // fall back to the plain error only if no such fallback was wired in.
      if (onCatalogMiss) onCatalogMiss(autoOpenFileId);
      else onError?.(`No NPC/monster model found for file id ${autoOpenFileId}`);
      return;
    }
    const { entry } = hit;
  ...
  }, [autoOpenFileId, autoOpenReady, allEntries, onSelectEntry, onError, onCatalogMiss]);

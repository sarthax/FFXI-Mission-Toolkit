...
export default function App({ launch = null }) {
  ...
  const [leftView, setLeftViewState] = useState(() => {
    ...
    // Same deal for an `?npc=` deep link -- the NPC page is the initial view,
    // not a switch, so NpcList mounts (and can auto-select) on first render.
    if (launch?.npcFileId != null) return 'npc';
  ...
  // Left explorer panel (zones/files/…); toolbar toggle, persisted. An `?npc=`
  // deep link needs the NPC list panel actually mounted to auto-select into,
  // regardless of whether the user had it closed on a previous run.
  const [explorerOpen, setExplorerOpen] = useState(
    () => (launch?.npcFileId != null ? true : localStorage.getItem('explorer') !== '0'));
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // `?npc=<fileId>` deep link: leftView is already 'npc' (set above),
        // NpcList mounts on its own and auto-selects once its list loads and
        // gamePath (settingsRef.current, just set above) is ready -- nothing
        // else to do here, and the "last opened DAT" fallback below must NOT
        // run instead, or it would load the previous session's model right
        // before NpcList's own selection replaces it.
        if (launchRef.current?.npcFileId != null) return;
  ...
  // `?npc=<fileId>` deep link (see ui/js/launch.js), when the file id isn't one of the ~2000
  // entries NpcList's own curated lists/npcs.json happens to catalog by name. Real DAT files at
  // that id can still exist -- npcs.json is a hand-curated "known interesting NPCs" list, not
  // every registered file -- so fall back to the same real FTABLE/VTABLE resolution
  // loadMergedTables() already does for the Data-view's zone cross-referencing, and open the raw
  // DAT directly via loadFromTree (the same path a manual FileTree click takes), rather than
  // reporting "not found" for something that's actually on disk.
  const resolveNpcFileIdFallback = useCallback(async (fileId) => {
    const settings = settingsRef.current;
    if (!settings?.gamePath) return;
    try {
      const { byFid } = await loadMergedTables(settings, dataTablesRef);
      const rel = byFid.get(fileId);
      if (!rel) {
        setStatusText(`No NPC/monster model found for file id ${fileId} -- not in the curated `
          + `NPC list, and no real file is registered at that id on this install either.`);
        return;
      }
      // UNCONFIRMED MATCH: file id ${fileId} isn't in the curated NPC list, so this is whatever
      // real file the client's own file table happens to have registered at that raw id -- NOT a
      // confirmed match for the entity that was actually asked for. A real example found this
      // wrong: a caller passed a Lamia's computed file id and got a Galka gear DAT back, because
      // the *source* modelid feeding that computation was itself bad data (see the caller's own
      // notes on where its ids come from), not because this lookup malfunctioned -- but from
      // here there is no way to tell "your input id was wrong" apart from "the file table moved
      // things around". Surfaced loudly rather than silently, so it never reads as a confirmed
      // "found it".
      setStatusText(`⚠ file id ${fileId} is NOT in the curated NPC list. Showing whatever real `
        + `file is registered at that raw id instead (${rel}) -- this is NOT a confirmed match `
        + `for the entity you were looking for, just the closest thing this tool could find.`);
      await loadFromTree(`${settings.gamePath}\\${rel}`);
    } catch (err) {
      setStatusText(`Could not resolve file id ${fileId}: ${err.message ?? err}`);
    }
  }, [loadFromTree]);
    ...
    <>
      ...
      {explorerOpen && leftView === 'npc' && (
        <NpcList
          ...
          selectedPath={selectedDat}
          onError={(msg) => setStatusText(msg)}
          autoOpenFileId={launchRef.current?.npcFileId ?? null}
          autoOpenReady={!!settings?.gamePath}
          onCatalogMiss={resolveNpcFileIdFallback}
        />
      )}

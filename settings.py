...
DEFAULTS = {
    ...
    "backport_root": "",           # empty = backport tooling disabled, see get_backport_root()
...
def get_backport_root() -> Path | None:
    """The Topaz-Assault-Backport checkout root (holds mission-packages/, dsp-engine-changes/,
    reports/) -- a real repo the user already has somewhere on disk, same no-default-no-guess
    convention as get_dsp_root(). Returns None (not a guessed path) when unset, so callers can
    tell "not configured" apart from "configured but wrong" and give a clear error instead of
    silently trying a path that doesn't exist on this machine."""
    con = sqlite3.connect(str(DB_PATH))
    try:

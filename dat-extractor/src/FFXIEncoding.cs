        ...
        public override string GetString(byte[] bytes, int index, int count)
        {
            ...
            for (int pos = index; pos < index + count; ++pos)
            {
                ...
                if (bytes[pos] == 0xFD && pos + 5 < index + count && bytes[pos + 5] == 0xFD)
                {
                    ...
                    // Original calls FFXIResourceManager.GetResourceString(ResourceID) here, which
                    // needs the full item/spell/key-item DAT lookup chain -- out of scope for this
                    // standalone extractor. Shows the raw resource ID instead of the resolved name.
                    DecodedString += String.Format("{0}[{1:X8}] Resource#{1:X8}{2}", FFXIEncoding.SpecialMarkerStart, ResourceID,

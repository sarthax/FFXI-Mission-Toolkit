...
def main():
    with tempfile.TemporaryDirectory() as td:
        ...
        # Minimal package files required by the cohesion gate.
        ...
        pending=assess_patch_lifecycle(package,target)
        assert pending.status=="AWAITING_APPROVAL",pending

        approved=json.loads(request.content)
        approved["status"]="APPROVED"
        eligible=assess_patch_lifecycle(package,target,approval_record=approved)
        assert eligible.status=="ELIGIBLE_FOR_DETERMINISTIC_APPLY",eligible

        (target/"x.lua").write_text("changed\n",encoding="utf-8")
        drifted=assess_patch_lifecycle(package,target,approval_record=approved)
        assert drifted.status=="DRIFTED",drifted

    print("patch lifecycle self-test: PASS")
